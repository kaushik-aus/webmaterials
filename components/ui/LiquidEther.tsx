...
export interface LiquidEtherProps {
  ...
  baseOpacity?: number; // guarantees a small visible alpha
}
...
// Cyan → Violet → Soft white
const defaultColors = ["#16E3FF", "#8A5BFF", "#F0E7FF"];
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    // Tiny alpha floor so it’s visible even with low velocity
    const bgVec4 = new THREE.Vector4(
      0,
      0,
      0,
      THREE.MathUtils.clamp(baseOpacity, 0, 1)
    );
    ...
    // Global last interaction (for autostart)
    ...
    class MouseClass {
      ...
      isHoverInside = true; // treat whole viewport as active background
      isAutoActive = false;
      autoIntensity = 2.0;
      takeoverDuration = 0.25;
      onInteract: (() => void) | null = null;

      // We attach to window to receive events even with pointer-events: none on the layer
      private _onMove = (e: MouseEvent | Touch) => {
        if (!this.container) return;
        const rect = this.container.getBoundingClientRect();
        ...
        // Track if pointer is within the LiquidEther wrapper bounds
        this.isHoverInside =
          clientX >= rect.left &&
          clientX <= rect.right &&
          clientY >= rect.top &&
      ...
      init(container: HTMLElement) {
        ...
        // Seed position near top-left so motion is visible instantly
        this.coords.set(-0.6, 0.3);
        this.coords_old.copy(this.coords);

        // Store bound refs to remove later
        (this as any)._listeners = { onMouseMove, onTouchMove };
      }

      dispose() {
        const ls = (this as any)._listeners;
        if (ls) {
          window.removeEventListener("mousemove", ls.onMouseMove);
          window.removeEventListener("touchmove", ls.onTouchMove);
        }
      }
    ...
    class AutoDriver {
      ...
      update() {
        ...
        // If pointer is inside, let user control take over; otherwise keep auto
        if (Mouse.isHoverInside && !this.active) return;
        if (!this.active) {
          this.active = true;
    ...
    const color_frag = `
      ...
      void main(){
        ...
        float outA = max(bgColor.a, lenv); // keep a minimum alpha floor
    ...
    class Advection extends ShaderPass {
      ...
      init() {
        ...
        this.line.visible = false; // keep clean
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        // Clamp the force area within bounds
        const cx = Math.min(
    ...
    class Simulation {
      ...
      // Use HalfFloat for broad support
      getFloatType() {
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      init() {
        ...
        // Seed so you see motion instantly even before interaction
        Mouse.coords.set(-0.5, 0.35);
